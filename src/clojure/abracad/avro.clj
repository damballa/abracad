...
(defn ^:private clj->json
  ...
  [schema]
  (json/generate-string (postwalk schema-mangle schema)))
...
(defn ^:private codec-for
  ...
  {:tag `CodecFactory}
  [codec]
  (if-not (string? codec) codec (CodecFactory/fromString codec)))
...
already a parsed schema, will first normalize and parse it."
  [schema]
  (-> schema parse-schema str (json/parse-string true)))
...
but the first `n` fields when sorting."
  [n schema]
  (-> schema unparse-schema (update-in [:fields] order-ignore n)))
...
(defmacro ^:private decoder-factory
  ...
  [method & args]
  `(. (DecoderFactory/get) ~method ~@args))
...
(defn direct-binary-decoder
  ...
  {:tag `Decoder}
  [source]
  (decoder-factory directBinaryDecoder source nil))
...
(defmacro ^:private encoder-factory
  ...
  [method & args]
  `(. (EncoderFactory/get) ~method ~@args))
...
(defn binary-encoder
  ...
  {:tag `Encoder}
  [sink]
  (encoder-factory binaryEncoder sink nil))
...
(defn direct-binary-encoder
  ...
  {:tag `Encoder}
  [sink]
  (encoder-factory directBinaryEncoder sink nil))
...
(defn ^:private avro-reader-var
  [sym]
  (intern (create-ns (symbol (namespace sym))) (symbol (name sym))))
